...
class KNN_Language:
    ...
    def driver(self):
        ...
        for i in data['Language']:
            lang_sentences.append(i)


        data_sen = {'Text': sentences,
                'Language': lang_sentences}

        df_sentences = pd.DataFrame(data_sen)
        ...
        testlabel = le.fit_transform(df_sentences['Language'])

       #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~


        #creates train-test splits
        image_train, image_test, label_train, label_test = train_test_split(testcase, testlabel, test_size=0.3, random_state=42)
        ...
        for i in neighbor_list:
            ...
            #LR = LogisticRegression(random_state=42, solver='newton-cg', multi_class='multinomial').fit(image_train, label_train)

            model.fit(image_train, label_train)


            #save the model into directory 
            with open('KNNSen' + str(i) + '_model.pickle', 'wb') as handle:
                pickle.dump(model, handle, protocol=pickle.HIGHEST_PROTOCOL)

        #save the dictionary with language and numeric values
            with open('KNNSen' + str(i) + '_labels.pickle', 'wb') as handle:
